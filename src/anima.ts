...
export class Anima {
  ...
  /**
   * snapshot - export the full agent state as a JSON-serializable object.
   * 
   * Useful for backups, migrations, cloning agents, or transferring
   * identity between storage backends.
   * 
   * ```typescript
  ...
  async snapshot(): Promise<{
    ...
    exportedAt: string;
    session: string;
    identity: ReturnType<IdentityManager['get']>;
    memories: Memory[];
    opinions: Opinion[];
    ...
    const relationships = this.relationships.getAll();
    const [memories, opinions, episodes, lifeboat] = await Promise.all([
      this.memory.getAllMemories(),
      this.memory.getOpinions(),
      this.episodes.recent(10000),
      this.memory.readLifeboat(),
    ]);
    ...
    return {
      ...
      relationships,
      episodes,
      lifeboat: lifeboat && lifeboat.trim().length > 0 ? lifeboat : null,
    };
  }

  /**
   * restore - import agent state from a snapshot.
   * 
   * Writes identity, memories, opinions, and relationships from a
   * previously exported snapshot. Call boot() after restore.
   * 
   * ```typescript
   * const state = JSON.parse(fs.readFileSync('backup.json', 'utf-8'));
   * await anima.restore(state);
   * const ctx = await anima.boot();
   * ```
   */
  async restore(snapshot: {
    identity?: Record<string, any>;
    memories?: Array<{ content: string; importance?: Memory['importance']; tags?: string[] }>;
    opinions?: Array<{ topic: string; current: string; confidence?: number }>;
    relationships?: Array<{ name: string; type?: Relationship['type']; context?: string }>;
  }): Promise<{ restored: { identity: boolean; memories: number; opinions: number; relationships: number } }> {
    const result = { identity: false, memories: 0, opinions: 0, relationships: 0 };

    if (snapshot.identity) {
      await this.identity.update(snapshot.identity, 'Restored from snapshot');
    ...
    if (snapshot.opinions) {
      for (const op of snapshot.opinions) {
        await this.memory.opine({ topic: op.topic, opinion: op.current, confidence: op.confidence ?? 0.7 });
        result.opinions++;
      }
    }
    ...
    if (snapshot.relationships) {
      for (const rel of snapshot.relationships) {
        await this.relationships.meet({ name: rel.name, type: rel.type, context: rel.context });
      }
      result.relationships = snapshot.relationships.length;
    }
